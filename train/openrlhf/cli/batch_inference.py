...
def batch_generate_vllm(args):
    ...
    prompts = []
    labels = []
    for prompt, label in prompts_dataset:
    ...
    for idx, output in enumerate(outputs):
        ...
        label = labels[idx % len(prompts)]
        output_dataset.append({"input": prompt, "output": output, "label": label})
...
def custom_rm_inference(args):
    ...
    if args.remote_rm_url:
        print(f"Loading custom `reward_func(queries, prompts, labels)` from {args.remote_rm_url}")
        import importlib.util
        spec = importlib.util.spec_from_file_location("reward_func", args.remote_rm_url)
        reward_module = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(reward_module)
        custom_reward_func = reward_module.reward_func

    pbar = tqdm(dataset, desc="Rewarding")
    output_dataset = []
    for item in pbar:
        ...
        reward = custom_reward_func(prompt, output, label)
    ...
    if args.post_processor and args.post_processor != "null":
        print(f"Use Processor {args.post_processor}, Reward Norm {args.normalize_reward}")
        processor = get_processor(args.post_processor)
        output_dataset = processor(args, output_dataset)
